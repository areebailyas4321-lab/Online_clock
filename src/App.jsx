...
function App() {
  ...
  // Debug: Monitor showClockOptions state
  useEffect(() => {
    console.log('showClockOptions changed:', showClockOptions);
  }, [showClockOptions]);

  const toggleClockMode = (mode) => {
    console.log('toggleClockMode called with:', mode);
    setClockMode(mode);
    ...
    if (themeMode === 'live') {
      const updateLiveTheme = () => {
        ...
        if (hour >= 5 && hour < 12) {
          ...
          newTheme = 'dark'; // Sunset/Evening often dark mode fits better or a specific sunset theme
        }
